...
# Evals Roles Setup Script
# This script creates the GitHub Actions evals IAM role for running evaluations
# All configuration is read from infra.yaml and secrets file
#
# Usage Examples:
#   # Set up evals role in dev account
#   ./scripts/setup/setup_evals_roles.sh dev
#
#   # Set up with auto-confirmation
#   ./scripts/setup/setup_evals_roles.sh dev -y
...
STACK_NAME=$(get_role_stack_name "evals" "$ENVIRONMENT")
TEMPLATE_FILE=$(get_role_template "evals")
...
deploy_stack() {
    ...
    # Show summary
    echo ""
    print_info "Deploying evals role with:"
    ...
    if [ "$AUTO_CONFIRM" = false ]; then
        confirm_deployment "Deploy evals role?" || exit 0
    fi
...
EOF
    ...
    print_complete "Evals role created successfully"
    echo ""
    print_info "Role ARN: $role_arn"
    echo ""
    print_info "Add this to your GitHub repository secrets:"
    print_info "  AWS_EVALS_ROLE_ARN_${ENVIRONMENT^^}=$role_arn"
}

delete_stack() {
    print_warning "Deleting evals role stack: $STACK_NAME"
    print_warning "WARNING: This will break GitHub Actions evaluations!"
    
    if [ "$AUTO_CONFIRM" = false ]; then
        confirm_destructive_action "$ENVIRONMENT" "delete evals role" || exit 0
    fi
...
print_complete "Evals role operation completed"
